        ...
        <section className={`pb-20  bg-gray-50 dark:bg-${Config.DarkColorProject} md:px-0 px-3`}>
            <div className="max-w-7xl mx-auto ">
                <div className="flex justify-between items-end mb-12">
                    ...
                    </div>
                    {
                        pathname !== '/celebrations' && (
                            <button className={`${Config.TextColor} font-semibold hover:underline`}>Ҳамаи хабарҳо →</button>
                        )
                    }
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {news.map((item) => (
                        <div key={item.id} className={`bg-white dark:bg-[#0f0f0f] rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-shadow duration-300 group cursor-pointer`}>
